...
export default function ClaaseOhadaPage() {
    // ---- États ----
    ...
    const loadDatas = async () => {
        setLoading(true);
        try {
            const res = await fetchItems<TypeOperation>("/fetch_classecompte", {
                page: currentPage,
                limit,
                query: search,
            });
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!formData.nom_classe || !formData.numero_classe) {
        ...
        try {
            const res = await saveItem("/insert_classecompte", payload);
    ...
    const handleEdit = async (id: number) => {
        setLoading(true);
        const res = await fetchSigleItem<TypeOperation>("/fetch_single_classecompte", id);
    ...
    const handleDelete = async (id: number) => {
        ...
        if (!confirm) return;
        const res = await removeItem("/delete_classecompte", id);
        showSuccessMessage(res);
        loadDatas();
    };
    ...
    return (
        <div className="col-md-12">
            ...
            {/* Modal */}
            <Modal
                title={isEditing ? "Modifier la classe" : "Ajouter la classe"}
                show={showModal}
                onClose={closeModal}
            >
                <form onSubmit={handleSubmit}>
                    <TextField
                        ...
                        onChange={(e) =>
                            setFormData({ ...formData, nom_classe: e.target.value })
                        }
                        required
                    />
                    ...
                    <TextField
                        ...
                        onChange={(e) =>
                            setFormData({ ...formData, numero_classe: e.target.value })
                        }
                        required
                    />
