...
interface TypeOperation {
    id?: number;
    nom_typeoperation?: string;
    author?: string;
    created_at?: string;
}

export default function TypeOperationPage() {
    // ---- États ----
    ...
    const loadDatas = async () => {
        setLoading(true);
        try {
            const res = await fetchItems<TypeOperation>("/fetch_typeoperation", {
                page: currentPage,
                limit,
                query: search,
            });
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const res = await saveItem("/insert_typeoperation", payload);
    ...
    const handleEdit = async (id: number) => {
        setLoading(true);
        const res = await fetchSigleItem<TypeOperation>("/fetch_single_typeoperation", id);
    ...
    const handleDelete = async (id: number) => {
        ...
        if (!confirm) return;
        const res = await removeItem("/delete_typeoperation", id);
        showSuccessMessage(res);
        loadDatas();
    };
    ...
    return (
        <div className="col-md-12">
            <h4>Liste des Types d'Opérations</h4>
            ...
            {/* Modal */}
            <Modal
                title={isEditing ? "Modifier Type d'Opération" : "Ajouter Type d'Opération"}
                show={showModal}
                onClose={closeModal}
            >
                <form onSubmit={handleSubmit}>
                    <TextField
                        name="nom_typeoperation"
                        label="Désignation"
                        icon="fas fa-cogs"
                        placeholder="Designation"
                        value={formData.nom_typeoperation ?? ""}
                        onChange={(e) =>
                            setFormData({ ...formData, nom_typeoperation: e.target.value })
                        }
                        required
                    />
