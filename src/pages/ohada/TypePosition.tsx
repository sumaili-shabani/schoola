...
export default function TypePositionPage() {
    // ---- États ----
    ...
    const loadDatas = async () => {
        setLoading(true);
        try {
            const res = await fetchItems<TypeOperation>("/fetch_typeposition", {
                page: currentPage,
                limit,
                query: search,
            });
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const res = await saveItem("/insert_typeposition", payload);
    ...
    const handleEdit = async (id: number) => {
        setLoading(true);
        const res = await fetchSigleItem<TypeOperation>("/fetch_single_typeposition", id);
    ...
    const handleDelete = async (id: number) => {
        ...
        if (!confirm) return;
        const res = await removeItem("/delete_typeposition", id);
        showSuccessMessage(res);
        loadDatas();
    };
    ...
    return (
        <div className="col-md-12">
            <h4>Liste des Types de positionnement</h4>
            ...
            {/* Modal */}
            <Modal
                title={isEditing ? "Modifier Le positionnement" : "Ajouter Le positionnement"}
                show={showModal}
                onClose={closeModal}
            >
                <form onSubmit={handleSubmit}>
                    <TextField
                        name="nom_typeposition"
                        label="Désignation"
                        icon="fas fa-cogs"
                        placeholder="Designation"
                        value={formData.nom_typeposition ?? ""}
                        onChange={(e) =>
                            setFormData({ ...formData, nom_typeposition: e.target.value })
                        }
                        required
                    />
