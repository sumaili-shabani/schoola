...
export default function TypeComptePage() {
    // ---- États ----
    ...
    const loadDatas = async () => {
        setLoading(true);
        try {
            const res = await fetchItems<TypeOperation>("/fetch_typecompte", {
                page: currentPage,
                limit,
                query: search,
            });
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const res = await saveItem("/insert_typecompte", payload);
    ...
    const handleEdit = async (id: number) => {
        setLoading(true);
        const res = await fetchSigleItem<TypeOperation>("/fetch_single_typecompte", id);
    ...
    const handleDelete = async (id: number) => {
        ...
        if (!confirm) return;
        const res = await removeItem("/delete_typecompte", id);
        showSuccessMessage(res);
        loadDatas();
    };
    ...
    return (
        <div className="col-md-12">
            <h4>Liste des Types de compte</h4>
            ...
            {/* Modal */}
            <Modal
                title={isEditing ? "Modifier Type de compte" : "Ajouter Type de compte"}
                show={showModal}
                onClose={closeModal}
            >
                <form onSubmit={handleSubmit}>
                    <TextField
                        ...
                        value={formData.nom_typecompte ?? ""}
                        onChange={(e) =>
                            setFormData({ ...formData, nom_typecompte: e.target.value })
                        }
                        required
                    />
