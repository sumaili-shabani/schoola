...
export default function SectionPage() {
    ...
    const loadDatas = async () => {
        setLoading(true);
        try {
            const res = await fetchItems<Tranche>("/fetch_section", {
                page: currentPage,
                limit,
                query: search,
            });
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const res = await saveItem("/insert_section", payload);
    ...
    const handleEdit = async (id: number) => {
        const res = await fetchSigleItem<Tranche>("/fetch_single_section", id);
    ...
    const handleDelete = async (id: number) => {
        ...
        if (!confirm) return;
        const res = await removeItem("/delete_section", id);
        showSuccessMessage(res);
        loadDatas();
    };
    ...
    return (
        <div className="col-md-12">
            ...
            {/* Modal */}
            <Modal
                title={isEditing ? "Modifier Section" : "Ajouter Section"}
                show={showModal}
                onClose={closeModal}
            >
                <form onSubmit={handleSubmit}>
                    <TextField
                        ...
                        onChange={(e) =>
                            setFormData({ ...formData, nomSection: e.target.value })
                        }
                        required
