...
export default function UserPage() {
    ...
    const handleEdit = async (id: number) => {
        try {
            setLoading(true);
            const roles = await fetchSigleItem<User[]>("/fetch_single_user", id);
    ...
    const handleDelete = async (id: number) => {
        ...
        if (confirmed) {
            try {
                setLoading(true);
                const res = await removeItem("/delete_user", id);
                setLoading(false);
                showSuccessMessage(res);
                loadDatas();
 ...
 */
    ...
    */
    ...
    const handleSubmitRole = async (e: React.FormEvent) => {
        e.preventDefault();
        setLoading(true);
        await saveItem('/change_pwd_user', formData);
        loadDatas();
        setLoading(false);
    ...
    const changeEtatCompte = async (id: number, etat: number) => {
        setLoading(true);
        const res = await fetchListItems(`/checkEtat_compte/${id}/${etat}`);
    ...
    */
    return (
        <div className="col-md-12 mt-0">
            <h4 className="mb-3">Gestion de compte utilisateur</h4>
            {/* loading component */}
            <LoaderAndError
                loading={loading}
                error={error}
                                ...
                                {datas.length === 0 ? (
                                    ...
                                    datas.map((item) => (
                                        <tr key={item.id}>
                                            ...
                                            <td>
                                                <button onClick={() => changeEtatCompte(item.id!, item.active!)} className={item.active === 1 ? 'btn btn-success btn-pill  btn-sm' : 'btn btn-danger btn-pill btn-sm'}>
                                                    <i className={item.active === 1 ? 'fas fa-check' : 'fas fa-close'}></i> {item.active === 1 ? 'Actif' : 'Inactif'}
                                                </button>

                                            </td>
                                            <td>
                                                <span className={item.id_role === 1 ? "badge bg-success" : item.id_role === 2 ? "badge bg-primary" : 
                                                    item.id_role === 3 ? "badge bg-dark" : item.id_role === 4 ? "badge bg-info" : item.id_role === 5 ? "badge bg-secondary" : item.id_role === 6 ? "badge bg-warning": "badge bg-danger"}>{truncateText(item.role_name ?? '', 20)}</span>
