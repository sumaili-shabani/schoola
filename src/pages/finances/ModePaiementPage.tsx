...
export default function ModePaiementPage() {
    // ---- États ----
    const [datas, setDatas] = useState<ModePaiement[]>([]);
    const [formData, setFormData] = useState<Partial<ModePaiement>>({});
    ...
    const loadDatas = async () => {
        setLoading(true);
        try {
            const res = await fetchItems<ModePaiement>("/fetch_modepaie", {
                page: currentPage,
                limit,
                query: search,
            });
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const res = await saveItem("/insert_modepaie", payload);
    ...
    const handleEdit = async (id: number) => {
        const res = await fetchSigleItem<ModePaiement>("/fetch_single_modepaie", id);
        const data = Array.isArray(res) && res.length ? res[0] : undefined;
        if (!data) {
            showErrorMessage("Mode de paiement introuvable !");
            return;
        }
        setFormData(data);
        setIsEditing(true);
    ...
    const handleDelete = async (id: number) => {
        ...
        if (!confirm) return;
        const res = await removeItem("/delete_modepaie", id);
        showSuccessMessage(res);
        loadDatas();
    };
    ...
    return (
        <div className="col-md-12">
            ...
            {/* Modal */}
            <Modal
                title={isEditing ? "Modifier Mode de Paiement" : "Ajouter Mode de Paiement"}
                show={showModal}
                onClose={closeModal}
            >
