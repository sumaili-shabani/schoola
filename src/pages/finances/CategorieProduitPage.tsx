...
export default function CategorieProduitPage() {
    // ---- États ----
    const [datas, setDatas] = useState<CategorieProduit[]>([]);
    const [formData, setFormData] = useState<Partial<CategorieProduit>>({});
    ...
    const loadDatas = async () => {
        setLoading(true);
        try {
            const res = await fetchItems<CategorieProduit>(
                "/fetch_categorie_produit",
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const res = await saveItem("/insert_categorie_produit", payload);
    ...
    const handleEdit = async (id: number) => {
        const res = await fetchSigleItem<CategorieProduit>(
            "/fetch_single_categorie_produit",
            id
        );
    ...
    const handleDelete = async (id: number) => {
        ...
        if (!confirm) return;
        const res = await removeItem("/delete_categorie_produit", id);
        showSuccessMessage(res);
        loadDatas();
    };
    ...
    return (
        <div className="col-md-12">
            ...
            {/* Modal */}
            <Modal
                title={isEditing ? "Modifier Catégorie" : "Ajouter Catégorie"}
                show={showModal}
                onClose={closeModal}
            >
